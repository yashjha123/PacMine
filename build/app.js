...
var muteSiren = function () {
    var audios = document.getElementsByClassName("siren");
    var i;
    for (i = 0; i < audios.length; i++) {
        audios[i].volume = 0;
    }
...
var muteDot = function () {
    var audios = document.getElementsByClassName("dot");
    var i;
    for (i = 0; i < audios.length; i++) {
        audios[i].volume = 0;
    }
...
var playOneS = function (sound) {
    console.log('playOneS', sound);
    var audio = document.getElementById('S' + sound);
    console.log(audio);
    if (audio) {
        audio.loop = false;
        audio.volume = 1;
...
var playGS = function (sound) {
    muteAll();
    playOS(sound);

}

var stopGS = function (sound) {
    muteAll();
}

var playOS = function (sound) {
    console.log('playOS', sound);
    var audio = document.getElementById('S' + sound);
    console.log(audio);
    if (audio) {
        audio.volume = 1;
...
var playDS = function (sound) {
    var audios = document.getElementsByClassName("dot");
    var i;
    for (i = 0; i < audios.length; i++) {
        if (audios[i].id != 'S' + sound) {
            audios[i].volume = 0;
        }
    }
        ...
        if (!this.fetchingAmbience && !this.cutscene) {
            ...
            if (this.masterVolume !== 0) {
                ...
                if (isSafari()) {
                    ...
                    request.addEventListener('load', function (e) {
                        context.decodeAudioData(this.response, function (buffer) {
                            ambS = context.createBufferSource();
                            ambS.buffer = buffer;
                            ambS.loop = true;
                            ambS.connect(context.destination);
                            ambS.start(0);
                        });
                    }, false);
                    request.send();
