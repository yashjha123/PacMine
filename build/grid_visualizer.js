...
function visualizeMaze(mazeArray) {
    ...
    for (let row = 0; row < mazeArray.length; row++) {
        ...
        for (let col = 0 ; col < mazeArray[row][0].length; col++) {
            ...
            let top_border = row == 0 ? true : mazeArray[row-1][0][col] == "X";
            let left_border = col == 0 ? true : mazeArray[row][0][col-1] == "X";
            let bottom_border = row == mazeArray.length-1 ? true : mazeArray[row+1][0][col] == "X";
            let right_border = col == mazeArray[row][0].length-1 ? true : mazeArray[row][0][col+1] == "X";
            // if (row == 0){
            let {
                border_top_width,
                border_right_width,
